...
exports.createEtudiant = async (req, res) => {
    try {
        ...
        //utliser une méthode de Mongoose pour vérifier l'existance d'un étudiant
        //avec le meme nom et prenom avant de le créer
...
exports.deleteEtudiant = async (req, res) => {
    try {
        ...
        // Trouver et modifier son activité false
        const etudiant = await Etudiant.findByIdAndUpdate(
            req.params.id,
            { actif: false },
            { new: true }
...
// Cette fonction permet de rechercher des étudiants dont le nom ou le prénom contient une chaîne de caractères donnée.
// Exemple: GET /api/etudiants/search
...
exports.searchEtudiants = async (req, res) => {
    try {
        ...
        if (!q) {
            return res.status(400).json({
                success: false,
                message: "Le champ de recherche est requis"
            });
        }
        console.log('🔎 Recherche par nom ou prénom:', q);
